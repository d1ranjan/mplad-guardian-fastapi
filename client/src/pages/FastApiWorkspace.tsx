...
export default function FastApiWorkspace() {
  ...
  const isWorkspaceRoute = ["/dashboard", "/projects", "/imports", "/alerts", "/allocation", "/models", "/team"].some(route => location === route || location.startsWith(`${route}/`));
...
function QuarkNavigation({ path, user }: { path: string; user: GuardianUser | null }) {
  ...
  const workspaceTarget = user ? "/dashboard" : "/#secure-access";
