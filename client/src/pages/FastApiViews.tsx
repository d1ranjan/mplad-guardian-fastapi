...
export function ImportsView({ token, user, onHydrate, setStatus }: { token: string; user: GuardianUser; onHydrate: () => Promise<void>; setStatus: (value: string) => void }) {
  ...
  const loadPresentationDemo = () => {
    setFile(presentationDemoFile());
    setIssues([]);
    setSummary(`${presentationDemoRecordCount} presentation-only synthetic records are selected. Validate them before importing.`);
  };
