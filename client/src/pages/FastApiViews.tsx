...
export function ImportsView({ token, user, onHydrate, setStatus }: { token: string; user: GuardianUser; onHydrate: () => Promise<void>; setStatus: (value: string) => void }) {
  ...
  return <section className="space-y-5"><div className="rounded-2xl border border-sky-200 bg-sky-50 p-6"><h2 className="font-semibold text-slate-900">Validate before importing</h2><p className="mt-1 text-xs leading-5 text-slate-600">CSV validation reports every available data-quality issue before a write is attempted.</p><div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-sm font-semibold text-amber-950">Presentation dataset</p><p className="mt-1 text-xs leading-5 text-amber-900/80">Import {presentationDemoRecordCount} explicitly synthetic records for a demonstration of imports, audit signals, model training, and review controls. They are fictional and are not MPLADS project evidence.</p>{user.role === "admin" && <button onClick={importPresentationDemo} disabled={busy} className="mt-3 rounded-lg border border-amber-400 px-3 py-2 text-sm font-semibold text-amber-950 disabled:opacity-50">{busy ? "Importing…" : "Import synthetic presentation data"}</button>}</div><input aria-label="Project CSV file" type="file" accept=".csv,text/csv" onChange={event => { setFile(event.target.files?.[0] ?? null); setIssues([]); setSummary(""); }} className="mt-4 block text-sm" />{user.role === "admin" ? <div className="mt-4 flex flex-wrap gap-3"><button onClick={validate} disabled={!file || busy} className="rounded-lg border border-[#1d5d8c] px-3 py-2 text-sm font-semibold text-[#1d5d8c] disabled:opacity-50">{busy ? "Working…" : "Validate my CSV"}</button><button onClick={submit} disabled={!file || busy || issues.some(issue => issue.severity === "error")} className="rounded-lg bg-[#1d5d8c] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Import my validated CSV</button></div> : <p className="mt-4 text-sm text-amber-800">Only administrators can validate and import project CSV files.</p>}{summary && <p className="mt-4 text-sm font-medium text-slate-700">{summary}</p>}</div>{issues.length > 0 && <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><SectionTitle title="Validation report" text="Errors block imports; warnings require analyst review." /><div className="divide-y divide-slate-100">{issues.map((issue, index) => <div key={`${issue.row}-${issue.field}-${index}`} className="flex gap-3 px-5 py-3 text-sm"><span className={`mt-0.5 rounded-full px-2 py-0.5 text-[11px] font-semibold ${issue.severity === "error" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}>{issue.severity}</span><p>Row {issue.row}{issue.field ? ` · ${issue.field}` : ""}: {issue.message}</p></div>)}</div></section>}</section>;
}
